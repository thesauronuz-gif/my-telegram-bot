...
from aiohttp import web  # Render portini ushlash uchun kerak

# BOT SOZLAMALARI (Sizning shaxsiy ma'lumotlaringiz 🚀)
...
async def main():
    ...
    # Render portini asinxron fonda ishga tushirish (Port Scan xatosini yo'qotadi)
    runner = web.AppRunner(app)
    await runner.setup()
    port = int(os.environ.get('PORT', 10000))
    site = web.TCPSite(runner, '0.0.0.0', port)
    await site.start()
    print(f"Render uchun veb-server {port}-portda yoqildi...")
